...
function loadPropertiesDiv(resourcePath, page) {
	...
	sessionAwareFunction(function() {
		new Ajax.Request('../registry_vault/properties-ajaxprocessor.jsp', {
			method : 'post',
			parameters : {
				path : resourcePath,
				page : page
			...
			function() {
				...
				if (reason == "") {
					propertyName = propertyName.replace(/^\s\s*/, '').replace(
							/\s\s*$/, '');
					if (propertyName == "") {
						reason = org_wso2_carbon_registry_secure_vault_ui_jsi18n["property.value.cannot.contain.only.white.spaces"];
					}
				...
				if (propertyName.startsWith("registry.")) {
					reason = org_wso2_carbon_registry_secure_vault_ui_jsi18n["property.name.cannot.be.hidden"];
				}
